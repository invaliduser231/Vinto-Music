type RetryOptions = {
  ...
  onRetry?: ((details: { attempt: number; delayMs: number; error: unknown }) => void) | null;
};
...
export async function withRetry<T>(fn: (attempt: number) => Promise<T>, options: RetryOptions = {}) {
