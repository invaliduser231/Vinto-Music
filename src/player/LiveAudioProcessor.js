...
function normalizePresetName(value, fallback = 'off') {
...
function buildProgram(filterPreset, eqPreset) {
  ...
  const eqGains = EQ_PRESETS[normalizedEq] ?? EQ_PRESETS.flat;
  ...
  return {
    ...
    panHz: isFiniteNumber(filterDef.panHz) ? filterDef.panHz : 0,
    filters,
  };
}

function processWithProgram(program, left, right, sampleIndex) {
  let outL = left;
  let outR = right;
  ...
  if (Array.isArray(program?.filters)) {
    for (const filter of program.filters) {
      [outL, outR] = filter.process(outL, outR);
    }
  }
